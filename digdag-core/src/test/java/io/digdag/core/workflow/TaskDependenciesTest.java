...
public class TaskDependenciesTest {
    ...
    @Test
    public void testChunkedParallelDependencies()
            throws Exception {
        StoredSessionAttemptWithSession attempt =
                runWorkflow(embed, projectPath, "chunked_parallel", loadYamlResource("/io/digdag/core/workflow/chunked_parallel.dig"));
        tm.begin(() -> {
            assertThat(attempt.getStateFlags().isSuccess(), is(true));
            List<ArchivedTask> tasks = localSite.getSessionStore().getTasksOfAttempt(attempt.getId());
            // # chunked_parallel.dig
            // +repeat:
            //   loop>: 5
            //   _chunked_parallel: 2
            //   _do:
            ...
            //
            // id | tasks index   | full_name                                 | expected parent_id | expected upstream_id 　
            // 1  | tasks.get(0)  | +chunked_parallel                         |                    |
            // 2  | tasks.get(1)  | +chunked_parallel+repeat                  | 1                  |
            // 3  | tasks.get(2)  | +chunked_parallel+repeat^sub              | 2                  |
            // 4  | tasks.get(3)  | +chunked_parallel+repeat^sub+loop-0       | 3                  |
            // 5  | tasks.get(4)  | +chunked_parallel+repeat^sub+loop-0+step1 | 4                  |
            // 6  | tasks.get(5)  | +chunked_parallel+repeat^sub+loop-0+step2 | 4                  | 5
            // 7  | tasks.get(6)  | +chunked_parallel+repeat^sub+loop-0+step3 | 4                  | 6
            // 8  | tasks.get(7)  | +chunked_parallel+repeat^sub+loop-1       | 3                  |
            // 9  | tasks.get(8)  | +chunked_parallel+repeat^sub+loop-1+step1 | 8                  | 9
            // 10 | tasks.get(9)  | +chunked_parallel+repeat^sub+loop-1+step2 | 8                  | 10
            // 11 | tasks.get(10) | +chunked_parallel+repeat^sub+loop-1+step3 | 8                  | 4,8
            // 12 | tasks.get(11) | +chunked_parallel+repeat^sub+loop-2       | 3                  |
            // 13 | tasks.get(12) | +chunked_parallel+repeat^sub+loop-1+step1 | 12                 |
            // 14 | tasks.get(13) | +chunked_parallel+repeat^sub+loop-1+step2 | 12                 | 13
            // 15 | tasks.get(14) | +chunked_parallel+repeat^sub+loop-1+step3 | 12                 | 14
            // 16 | tasks.get(15) | +chunked_parallel+repeat^sub+loop-3       | 3                  | 4,8
            // 17 | tasks.get(16) | +chunked_parallel+repeat^sub+loop-3+step1 | 16                 |
            // 18 | tasks.get(17) | +chunked_parallel+repeat^sub+loop-3+step2 | 16                 | 17
            // 19 | tasks.get(18) | +chunked_parallel+repeat^sub+loop-3+step3 | 16                 | 18
            // 20 | tasks.get(19) | +chunked_parallel+repeat^sub+loop-4       | 3                  | 12,16
            // 21 | tasks.get(20) | +chunked_parallel+repeat^sub+loop-4+step1 | 20                 |
            // 22 | tasks.get(21) | +chunked_parallel+repeat^sub+loop-4+step2 | 20                 | 21
            // 23 | tasks.get(22) | +chunked_parallel+repeat^sub+loop-4+step3 | 20                 | 22
